...
export APP_NAME=wordpress-task-sizer
...
# Process all YAML files in the template directory
for file in $(find kubernetes/template -type f -name '*.yaml'); do
  # Add the yaml separator if not present
  [[ $(head -c 3 "$file") != "---" ]] && echo '---'
  envsubst < "$file"
done
...
if [ "${USE_AUTOSCALER:-}" == "true" ]; then
  envsubst < ./kubernetes/extras/autoscaler.yaml
fi

# Handle public ingress
if [ -n "${PUBLIC_INGRESS_HOSTNAME:-}" ]; then
  envsubst < ./kubernetes/extras/public-ingress.yaml
fi

# Handle Pushgateway
if [ "${PUSHGATEWAY_ENABLED:-}" == "true" ]; then
  envsubst < ./kubernetes/extras/pushgateway.yaml
fi
